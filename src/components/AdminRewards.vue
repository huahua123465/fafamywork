...
<template>
  <fieldset class="admin-group reward-admin">
    ...
    <p class="admin-group-note">好友通过用户的专属链接注册新账号，分享者获得奖励。在此设置奖励、查看用户与积分流水。所有人工操作都会记录审计日志。</p>
    ...
    <template v-else>
      <div class="reward-settings">
        ...
        <label><span>每邀请一人奖励</span><input v-model.number="settings.pointsPerQualifiedVisit" type="number" min="0" /></label>
        <label><span>每日次数上限</span><input v-model.number="settings.dailyRewardLimit" type="number" min="0" /></label>
        <label><span>新用户注册奖励</span><input v-model.number="settings.registrationBonus" type="number" min="0" /></label>
      </div>

...
<template>
  <Modal :open="open" title="分享这个项目" @close="emit('close')"><div class="modal-body">
    ...
    <div v-else class="share-reward-box"><h3>专属积分分享</h3><p v-if="busy">正在生成专属链接…</p><p v-else-if="error" class="form-error">{{ error }}</p><template v-else-if="offer"><p>每次有效访问奖励 <strong>{{ offer.pointsPerVisit }}</strong> 积分，今天还可获得 <strong>{{ offer.remainingToday }}</strong> 次奖励。</p><ul class="share-rules"><li>好友打开你的专属链接，浏览满 {{ offer.qualificationSeconds }} 秒并滑动或点击页面，算一次有效访问</li><li>好友不需要注册或登录</li><li>同一位好友（同一网络或同一台设备）{{ offer.visitorDedupeDays }} 天内只算一次；你自己打开不计分</li></ul><button class="button" :disabled="offer.remainingToday <= 0" @click="rewardShare"><Icon :name="justDone && result?.kind === 'reward' ? 'success' : 'share'" :size="17" /> {{ offer.remainingToday <= 0 ? '今日奖励已达上限' : justDone && result?.kind === 'reward' ? '已复制' : '分享赚积分' }}</button></template></div>
    <div v-if="result" ref="resultBox" class="share-result" :class="{ error: result.outcome === 'error' }" role="status">
      ...
      <template v-else-if="result.kind === 'reward'"><strong>{{ result.outcome === 'copied' ? '专属链接已复制' : '已打开分享' }}</strong><p>{{ result.outcome === 'copied' ? '到微信或 QQ 里粘贴发给好友。' : '' }}好友打开后浏览满 {{ offer?.qualificationSeconds }} 秒并滑动页面，你就会获得 {{ offer?.pointsPerVisit }} 积分，可在「我的积分」里查看。</p></template>
